      ...
      //todo
{
    ...
    if (err != 0) {
        ...
        framebuffer_open(module, &fb_dev);
        ALOGE("Display framebuffer_open: %p", fb_dev);
    }
    //todo fb
}
...
//     args->win_num = windows.size();
    ...
    for (auto &win: windows) {
        if (win.contains_client_target()) {
            ...
            // }
            // if (fb_dev) {
            //     fb_dev->post(fb_dev, client_target.get_buffer_handle());
            //     ALOGE("fb_post");
            // } else {
            //     ALOGE("error: fb == null");
            // }
...
//             }
            // if (fb_dev) {
            //     fb_dev->post(fb_dev, layers.find(lyr_id)->second.get_buffer_handle());
            //     ALOGE("fb_post_layer");
            // } else {
            //     ALOGE("error: fb == null");
            // }
...
{
    ALOGE("retrieve_display_configs");
    ...
    //     }
        values[0] = 1920;
        values[1] = 1080;
        values[2] = 1000000000.0f / 60;
        values[3] = values[0] / 21.0f;
        values[4] = values[1] / 21.0f;
